#!/bin/bash
#
# Setup Claude Code UI with ngrok tunnel for auto-start on macOS boot
#
...
PLIST_LABEL="com.claude-code-ui.ngrok"
PLIST_FILE="$HOME/Library/LaunchAgents/${PLIST_LABEL}.plist"
SCRIPTS_HOME_DIR="$HOME/Scripts"

echo -e "${CYAN}--- Claude Code UI + ngrok Tunnel 設置腳本 (macOS) ---${NC}"
...
if [ "$ACTION" = "remove" ]; then
    ...
    # 刪除 LaunchAgent
    echo -e "\n${YELLOW}3. 正在刪除 macOS LaunchAgent...${NC}"
    if [ -f "$PLIST_FILE" ]; then
        rm -f "$PLIST_FILE"
...
RUNNER_EOF
    ...
    # 步驟 4: 創建 macOS LaunchAgent
    echo -e "\n${YELLOW}4. 正在配置 macOS LaunchAgent...${NC}"
...
PLIST_EOF
    ...
    echo -e "   ${GREEN}- 已創建 LaunchAgent 配置${NC}"
