#!/bin/bash
#
# Setup Claude Code UI with Tailscale Funnel for auto-start on Linux boot
#
...
SERVICE_NAME="claude-code-ui-funnel"
SERVICE_FILE="$SYSTEMD_DIR/${SERVICE_NAME}.service"
SCRIPTS_HOME_DIR="$HOME/Scripts"

echo -e "${CYAN}--- Claude Code UI + Tailscale Funnel 設置腳本 (Linux) ---${NC}"
...
if [ "$ACTION" = "remove" ]; then
    ...
    if [ -z "$PORT_PID" ]; then
        PORT_PID=$(fuser "$PORT/tcp" 2>/dev/null | xargs | awk '{print $1}' || true)
    fi
    if [ -n "$PORT_PID" ]; then
    ...
    # 刪除 systemd service
    echo -e "\n${YELLOW}5. 正在刪除 systemd service...${NC}"
    if [ -f "$SERVICE_FILE" ]; then
        rm -f "$SERVICE_FILE"
...
RUNNER_EOF
    ...
    # 步驟 7: 創建 systemd user service
    echo -e "\n${YELLOW}7. 正在配置 systemd user service...${NC}"
    mkdir -p "$SYSTEMD_DIR"

    cat > "$SERVICE_FILE" << SERVICE_EOF
[Unit]
Description=Claude Code UI + Tailscale Funnel
