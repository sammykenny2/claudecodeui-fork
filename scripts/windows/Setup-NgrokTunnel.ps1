...
.NOTES
    ...
    - Log file: $env:USERPROFILE\Scripts\claude-code-ui.log
#>
...
$runnerScript = Join-Path $userScriptsDir "StartClaudeCodeUI.ps1"
$logFile = Join-Path $userScriptsDir "claude-code-ui.log"
$startupFolder = [Environment]::GetFolderPath('Startup')
$startupShortcut = Join-Path $startupFolder "StartClaudeCodeUI.lnk"
...
if ($Remove) {
    ...
    if (-not $NonInteractive) {
        ...
        Write-Host "   - Runner 腳本 (StartClaudeCodeUI.ps1)" -ForegroundColor White
    ...
    if (Test-Path $runnerScript) {
        ...
        Write-Host "   - 已刪除 StartClaudeCodeUI.ps1 ✓" -ForegroundColor Green
    } else {
        Write-Host "   - StartClaudeCodeUI.ps1 不存在，跳過" -ForegroundColor Gray
    }
...
"@
    ...
    Write-Host "   - 已創建 StartClaudeCodeUI.ps1 ✓" -ForegroundColor Green
    ...
    $Shortcut.Description = "Claude Code UI + ngrok Tunnel Auto-Start"
    $Shortcut.Save()
